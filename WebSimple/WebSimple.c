...
This example displays a HTML string (in memory) in a browser object. This string contains a special
"URL" which begins with "app:". The rest of the URL is a number, and it refers to which one of our
presets HTML strings to display. In other words, we want to create web pages where the user can
click on a link to display another HTML string in memory (rather than loading a URL or some HTM file
on disk).

We have a bunch of HTML strings in memory. They are all stored in one buffer, and each HTML string
is nul-terminated. The buffer ends with an extra nul character. We've declared this as a global
buffer named HtmlStrings[]. The first string in the buffer is string #1. The second string is #2.
Etc. So, to link to HTML string #2 on a web page, our link would be

<A HREF="app:2">Click here for HTML string #2.</A>

To implement this, we utilize our IDocHostUIHandler's TranslateUrl() function. This is called by the
browser whenever the user clicks on a link, and gives us a chance to translate (ie, replace the URL).
In this function we check for any link that starts with "app:", and if that's what we have, we replace
that link with something harmless that the browser can't navigate to. Then we post a special message
to the window hosting the browser control to tell it which HTML string to display. The window simply
calls DisplayHTMLStr to do that.
...
const TCHAR HtmlStrings[] = { "<FONT COLOR=GREEN>This is page 1.</FONT><P><A HREF=\"app:2\">Click here.</A>\0\
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	if (OleInitialize(NULL) == S_OK)
	{
			...
			HWND_DESKTOP, NULL, hInstance, 0)))
		{
			...
			if (!(msg.hwnd = CreateWindow(&Button[0], "Back",
			...
			// Set its font
			SendMessage((HWND)msg.hwnd, WM_SETFONT, (WPARAM)FontHandle, 0);

			// Create a "Forward" button.
			if (!(msg.hwnd = CreateWindow(&Button[0], "Forward",
				WS_TABSTOP | BS_PUSHBUTTON | BS_TEXT | WS_CHILD | BS_VCENTER | WS_VISIBLE,
			...
			if (!(msg.hwnd = CreateWindow(&Button[0], "Stop",
				WS_TABSTOP | BS_PUSHBUTTON | BS_TEXT | WS_CHILD | BS_VCENTER | WS_VISIBLE,
				...
				MainWindow, (HMENU)1000, hInstance, 0)))
			{
				...
				DisplayHTMLStr(msg.hwnd, &HtmlStrings[0]);
