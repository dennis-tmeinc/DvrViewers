...
public:
	...
	// at the end.
	///////////////////////////////////////////////////
	IOleInPlaceFrameEx		frame;		// My IOleInPlaceFrame object. Must be first within my _IOleInPlaceFrameEx
...
public:
	...
	IDocHostUIHandlerEx	ui;			// My IDocHostUIHandler object. Must be first within my _IDocHostUIHandlerEx.
	IOleObject * browserObject;
...
IOleObject	* EmbedBrowser(HWND hwnd)
{
	...
	if (!CoGetClassObject(CLSID_WebBrowser, CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, NULL, IID_IClassFactory, (void **)&pClassFactory) && pClassFactory)
	{
		...
		if (!pClassFactory->CreateInstance(0, IID_IOleObject, (void **)&browserObject))
		{
			...
			iOleClientSiteEx->inplace.frame.window = hwnd;
			...
			if (!browserObject->SetClientSite((IOleClientSite *)iOleClientSiteEx))
			{
				...
				// you want.
				browserObject->SetHostNames(L"Localhost", 0);
				...
				if (!OleSetContainedObject((IUnknown *)browserObject, TRUE) &&
					...
					!browserObject->DoVerb(OLEIVERB_SHOW, NULL, (IOleClientSite *)iOleClientSiteEx, -1, hwnd, &rect) &&
...
{
	IOleClientSiteEx * iOleClientSiteEx;
	if (!browserObject->GetClientSite((IOleClientSite **)&iOleClientSiteEx)) {
