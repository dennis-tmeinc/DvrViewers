...
//	list of file ;
	...
	struct dvrtime m_filetime, m_curtime;
#ifdef CHECK_TIME
	struct dvrtime m_curtimesave ;	
#endif
	__int64 m_filetimestamp;
	CMpegPs m_ps;
...
//	unsigned long  m_filetimestamp ;		// first frame time stamp of the file, use for play time calculation
	...
	struct sync_timer {
		struct dvrtime m_synctime ; // reference filename time
		int			   m_insync ;			// indicate if m_synctime/m_synctimestamp pair is setup
		__int64		   m_synctimestamp ; // time stamp at m_synctime
	} m_sync ;							
	int m_maxframesize ;
	...
	// One PS = VideoES + AudioES + AudioEs + ... + AudioEs.
	// If another VideoES is encountered, save it for the next bundle
	char *m_backup;
...
public:
	...
	int getlocation(char *buf, int size);
