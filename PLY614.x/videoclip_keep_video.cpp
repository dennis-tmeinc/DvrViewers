...
int write_to_avifile(AVCodecContext *avc, FILE *fp, FILE *fpi, struct avi_info *ai, struct frame_cache *f);
...
void CALLBACK DecCBFunc(long nPort,char *pBuf,long nSize,FRAME_INFO *pFrameInfo,long nRes,long nRes2)
{
	...
	if (pFrameInfo->nType == T_AUDIO16) {
		...
		hs->v++;
		/* we got decoded video frame, add encoded video frame instead */
		struct frame_cache *f = unbuffer_v_frame(hs);
		if (f) {
			buffer_av_frame(hs, f);
		}
	}
}
						   ...
						   struct dvrtime endtime, int ch_progress_base, int ch_progress_max)
{
	...
	*ptr = '\0';
	...
	if( stream ) {
		...
		while( 1 ) {
			bool vfull;
			/* getdata only when v buffer or av buffer is not full */
			EnterCriticalSection(&g_hs[ch].cs);
			vfull = is_vbuffer_full(&g_hs[ch]);
			LeaveCriticalSection(&g_hs[ch].cs);
			if (!vfull) {
				int dataok = stream->getdata( &framedata, &framesize, &frametype, &headerlen );
				if (!dataok) {
					error++;
				...
				if ((frametype == FRAME_TYPE_KEYVIDEO) || (frametype == FRAME_TYPE_VIDEO)) {
					f = (struct frame_cache *)malloc(sizeof(int) * 2 + framesize);
					if (f) {
						PBYTE pData = (PBYTE)f + sizeof(int) * 2;
						f->len = remove_ps_header((PBYTE)framedata, framesize, pData, &ai.frameperiod, &ai.resolution);
						f->type = frametype;
						//TRACE(_T("--buffer:%d,%d\n"),f->type, f->len);
						buffer_v_frame(&g_hs[ch], f);
					}
					g_hs[ch].v_sent++;
			...
			if (f) {
				write_to_avifile(&avc, fp, fpi, &ai, f);
	...
	while ((f = unbuffer_v_frame(&g_hs[ch]))) {	
		write_to_avifile(&avc, fp, fpi, &ai, f);
		free(f);
	}
...
int write_to_avifile(AVCodecContext *avc,
