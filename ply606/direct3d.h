...
{
public:
	CDirect3D(tme_object_t *pIpc, HWND hwnd, video_format_t *p_fmt, int iPtsDelay, bool bResizeParent);
	virtual ~CDirect3D();
...
private:
	...
	D3DFORMAT Direct3DVoutSelectFormat( D3DFORMAT target, const D3DFORMAT *formats, size_t count);
	D3DFORMAT Direct3DVoutFindFormat(int i_chroma, D3DFORMAT target);
	int Direct3DVoutCreatePictures( size_t i_num_pics );
	int Direct3DVoutSetOutputFormat(D3DFORMAT format);
	void Direct3DVoutReleasePictures();
#ifdef USE_NEW3D
	int Direct3DVoutResetDevice();
#else
	int Direct3DVoutResetDevice(UINT i_width, UINT i_height);
#endif
	int Direct3DVoutCreateScene();
	void Direct3DVoutReleaseScene();
	void get_capability_for_osversion(void);
   ...
   volatile UINT16 m_iChanges;        /* changes made to the video display */
	...
	D3DFORMAT m_bbFormat;
	LPDIRECT3DTEXTURE9 m_d3dtex;
	LPDIRECT3DVERTEXBUFFER9 m_d3dvtc;
#if 0
