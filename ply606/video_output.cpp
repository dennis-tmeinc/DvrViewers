...
CVideoOutput::CVideoOutput(tme_object_t *pIpc, HWND hwnd, video_format_t *p_fmt, int iPtsDelay, bool bResizeParent)
{
	...
	* will be initialized later in InitThread */
    ...
    // init blur area
    m_ba_number=0 ;
    m_blur_save_pic.date=0 ;
    m_blur_save_pic.i_planes=0 ;
    m_blur_save_pic.b_force=0 ;
    m_blur_save_pic.i_status=0 ;
}
	...
	*/
	while( (!m_bDie) && (!m_bError) )
	{
		...
		*/
        ...
        if(  p_picture != NULL ) {
            if( m_blur_save_pic.i_status ) {            // paused
                if( p_picture != p_last_picture ) {
                    if( m_ba_number>0 ) {
                        backup_blur_pic(p_picture);
                        blurPicture(p_picture);
                    }
                    else {
                        clear_blur_pic();
                    }
                    ...
                    if( m_blur_save_pic.b_force ) {
                        if( m_blur_save_pic.i_planes>0 ) {
                            restore_blur_pic(p_picture);
                        }
                        else {
                            backup_blur_pic(p_picture);
                        }
            ...
            else if( m_ba_number>0 && p_picture != p_last_picture ) {
                blurPicture(p_picture);
            }
        }
...
void  CVideoOutput::clear_blur_pic()
{
    int i;
    for( i=0; i<m_blur_save_pic.i_planes; i++ ) {
        delete m_blur_save_pic.p[i].p_pixels ;
    }
    m_blur_save_pic.i_planes = 0;
}

void  CVideoOutput::backup_blur_pic(picture_t * pic)
{
    int i;
    clear_blur_pic();
    for( i=0; i<pic->i_planes; i++ ) {
        int planesize = pic->p[i].i_pitch * pic->p[i].i_lines ;
        m_blur_save_pic.p[i].i_pitch = pic->p[i].i_pitch ;
        m_blur_save_pic.p[i].i_lines = pic->p[i].i_lines ;
        m_blur_save_pic.p[i].i_visible_lines = pic->p[i].i_visible_lines ;
        m_blur_save_pic.p[i].i_visible_pitch = pic->p[i].i_visible_pitch ;
        m_blur_save_pic.p[i].p_pixels = new UINT8 [planesize] ;
        memcpy( m_blur_save_pic.p[i].p_pixels, pic->p[i].p_pixels, planesize );
    }
    m_blur_save_pic.i_planes = pic->i_planes ;
}

void  CVideoOutput::restore_blur_pic(picture_t * pic)
{
    int i;
    if( pic->i_planes!=m_blur_save_pic.i_planes ) {
        return ;
    }
    for( i=0; i<pic->i_planes; i++ ) {
        int planesize = pic->p[i].i_pitch * pic->p[i].i_lines ;
        memcpy( pic->p[i].p_pixels, m_blur_save_pic.p[i].p_pixels, planesize );
    }
}

// quick blur 
static void blur( unsigned char *pix, int pitch, int bx, int by, int bw, int bh, int radius, int shape ) 
{
    if (radius<1) return;
    int wm=bw-1;
    int hm=bh-1;
...
void CVideoOutput::blurPicture(picture_t * pic)
{
    ...
    if( pic->i_planes == 3 && pic->p[0].p_pixels && pic->p[1].p_pixels && pic->p[2].p_pixels ) {
        // YV12
        width = pic->p[0].i_visible_pitch / pic->p[0].i_pixel_pitch ;
        ...
        for( i=0;i<m_ba_number;i++) {
            ...
            if( bw>3 && bh>3 && radius>3 && radius<bw && radius<bh ) {
                blur( (unsigned char *)pic->p[0].p_pixels, pic->p[0].i_pitch, bx,   by,   bw,   bh, radius, m_blur_area[i].shape );
                blur( (unsigned char *)pic->p[1].p_pixels, pic->p[1].i_pitch, bx/2, by/2, bw/2, bh/2, radius/2, m_blur_area[i].shape );
                blur( (unsigned char *)pic->p[2].p_pixels, pic->p[2].i_pitch, bx/2, by/2, bw/2, bh/2, radius/2, m_blur_area[i].shape );
            }
        }
    }
}
