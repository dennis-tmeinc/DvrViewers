...
int	MpegPlayer::play() 
{
    ...
    // Play state for blur procedure
    struct vout_set *pvs, vs;
    pvs = getVoutSetForWindowHandle(m_playerwindow, &vs);
    if (pvs && vs.vout) {
        CVideoOutput *pVout = vs.vout;
        pVout->m_blur_save_pic.i_status = 0 ;
    }
...
int MpegPlayer::pause()
{
	if( m_playstat!=PLAY_STAT_PAUSE ) {					// already paused?
        ...
        // Paused state for blur procedure
        struct vout_set *pvs, vs;
        pvs = getVoutSetForWindowHandle(m_playerwindow, &vs);
        if (pvs && vs.vout) {
            CVideoOutput *pVout = vs.vout;
            pVout->m_blur_save_pic.i_status = 1 ;
        }
	}
	return 0;
}
...
int  MpegPlayer::setblurarea(struct blur_area * ba, int banumber )
{
    ...
    if (pvs && vs.vout) {
        ...
        pVout->m_blur_save_pic.b_force = (m_playstat == PLAY_STAT_PAUSE) ;    // blur area changed
        //  refreshdecoder();    
    }
    return 0;
}
