...
#define DVRFILE_606	1003	// 606

// Extra DVR file header
struct DVRFILEHEADEREX {
	int		size ;					// extra header size
	DWORD	flagex ;				// DVR file flag, 'DVEX'.
	int   platform_type ;			// platform type, support (DVRFILE_DUEL_MIPS, DVRFILE_606) only.
	...
	DWORD	date ;					// file start date in BCD, ex 2007-03-01 as 20070301
	DWORD   time ;					// file start time of the day, in seconds
	WORD	millisec;				// for milli second precision (time)
	WORD	len_millisec;			// for milli second precision (length)
...
protected:
	...
	//std::vector <index1_entry_t> m_keyindex ;		// list of key frame index
