/*
* SipleWeb.c -- display a web page in your own window.
*
...
// use getprop instead of getwindowlong
#define CWIN_WEBOBJ	( MAKEINTATOM(18) )
...
HRESULT WINAPI GetWebPtrs(HWND hwnd, IWebBrowser2 **webBrowser2Result, IHTMLDocument2 **htmlDoc2Result)
{
	...
	if (webBrowser2Result || htmlDoc2Result)
	{
		...
		if (!IsWindow(hwnd) ||
			...
			!(browserObject = *((IOleObject **)GetProp(hwnd, CWIN_WEBOBJ))) ||
...
void WINAPI UnEmbedBrowserObject(HWND hwnd)
{
	...
	if ((browserHandle = (IOleObject **)GetProp(hwnd, CWIN_WEBOBJ)))
	{
...
long WINAPI EmbedBrowserObject(HWND hwnd)
{
	...
	if (!CoCreateInstance(&CLSID_WebBrowser, 0, CLSCTX_INPROC, &IID_IWebBrowser2, (void **)&webBrowser2))
	{
		...
		if ((*((IOleObject **)ptr) = browserObject))
		{
			...
			SetProp(hwnd, CWIN_WEBOBJ, (HANDLE)ptr);
